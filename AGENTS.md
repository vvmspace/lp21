...
Пускай будет красивая иконка обратного отсчёта, когда все задачи выполнены.
...
## Статус прототипа (обновляется)

### Уже реализовано
- Данные состояния (ритуалы, логи, задачи, дневные циклы) живут в базе, а не в локальных файлах.
- Языки подтягиваются динамически из `/locales`, доступны через `GET /api/v1/language`.
- Смена языка авторизованного пользователя сохраняется в базе через `POST /api/v1/language`.
- Авторизация: если пароль неверный, пользователь не сохраняется на фронте.
- Приложение без авторизации использует данные гостя, кнопки остаются неактивными и ведут на логин.
- E2E падает при ошибках в браузерной консоли.

### Чего ещё не хватает (обязательные улучшения)
- Прогон прод-окружения в Docker перед релизом (и фиксирование результата в PR).
- Хранение истории диалогов с AI и метрик состояния (зачатки RAG по ТЗ).
- Более надёжная авторизация (токены/сессии), чтобы не хранить пароль в localStorage.
